...
// Bepaal het juiste afbeelding pad voor de hero
$heroImagePath = '';
if ($blog->image_path) {
    // Check of het path al volledig is of relatief
    if (strpos($blog->image_path, 'http') === 0) {
        $heroImagePath = $blog->image_path;
    } else {
        $heroImagePath = URLROOT . '/' . $blog->image_path;
    }
}

// Voor meta tags gebruiken we de hero image of een fallback
$pageImage = $heroImagePath ?: URLROOT . '/public/img/og-image.jpg';
...
<div style="position: fixed; top: 50px; left: 10px; background: black; color: white; padding: 10px; z-index: 9999; font-size: 12px; max-width: 300px;">
    <strong>Debug Info:</strong><br>
    blog->image_path: <?php echo htmlspecialchars($blog->image_path ?? 'NULL'); ?><br>
    heroImagePath: <?php echo htmlspecialchars($heroImagePath ?? 'NULL'); ?><br>
    URLROOT: <?php echo URLROOT; ?><br>
    File exists: <?php echo $blog->image_path && file_exists($blog->image_path) ? 'YES' : 'NO'; ?>
</div>
<?php endif; ?>
...
<main class="bg-gradient-to-br from-gray-50 via-blue-50/30 to-indigo-50/20 min-h-screen">
    ...
    <section class="relative text-white <?php echo $heroImagePath ? 'bg-cover bg-center' : 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900'; ?>" 
             style="<?php echo $heroImagePath ? 'background-image: url(\'' . $heroImagePath . '\'); background-size: cover; background-position: center; min-height: 50vh;' : 'min-height: 50vh;'; ?>"
             <?php if (isset($_GET['debug'])): ?>
             data-debug-image="<?php echo htmlspecialchars($heroImagePath ?? 'none'); ?>"
             <?php endif; ?>>
        <!-- Overlay -->
        <div class="absolute inset-0 <?php echo $heroImagePath ? 'bg-black/60' : ''; ?>">
                    ...
                    <div class="relative">
                        <?php if ($blog->video_path): ?>
                            ...
                            <div class="relative aspect-video bg-black">
                                <video controls class="w-full h-full rounded-t-2xl sm:rounded-t-3xl" poster="<?php echo $heroImagePath; ?>">
