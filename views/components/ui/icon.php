<?php
/**
 * Lucide icon renderer (inline SVG).
 *
 * Usage:
 *   pp_icon('arrow-right', ['size' => 20, 'class' => 'text-hague']);
 *   pp_icon('arrow-right', 20);
 *
 * Bron: lucide.dev (ISC license). Curated subset; uitbreidbaar door
 * extra paths toe te voegen aan $LUCIDE_PATHS.
 */

if (!function_exists('pp_icon')) {

    /**
     * @var array<string, string> Map van icon-naam naar SVG inner paths.
     *   Stroke is 1.75, geen fills, gebruik `currentColor`.
     */
    $GLOBALS['LUCIDE_PATHS'] = [
        // Navigation / chrome
        ...
        'thumbs-up' => '<path d="M7 10v12"/><path d="M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H7"/><path d="M7 10H4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h3"/><path d="M15 5.88V3a2 2 0 0 0-4 0"/>',
        'thumbs-down' => '<path d="M17 14V2"/><path d="M9 18.12 10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H17"/><path d="M17 14h3a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2h-3"/><path d="M9 18.12V21a2 2 0 0 0 4 0"/>',
        'user-x' => '<path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="17" y1="8" x2="22" y2="13"/><line x1="22" y1="8" x2="17" y2="13"/>',
        ...
        'scale' => '<path d="m16 16 3-8 3.001 8A5.002 5.002 0 0 1 16 16Z"/><path d="m2 16 3-8 3.001 8A5.002 5.002 0 0 1 2 16Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/>',
        ...
        'sparkles' => '<path d="M12 3l1.45 4.55L18 9l-4.55 1.45L12 15l-1.45-4.55L6 9l4.55-1.45z"/><path d="M5 3v4"/><path d="M19 17v4"/><path d="M3 5h4"/><path d="M17 19h4"/>',
        ...
        'leaf' => '<path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19.2 2.96c1 6.04.43 11.32-2.78 14.95A6.97 6.97 0 0 1 11 20z"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/>',
        'recycle' => '<path d="M7 19H4.815a1.83 1.83 0 0 1-1.57-.881 1.785 1.785 0 0 1-.004-1.784L7.196 9.5"/><path d="M11 19h8.203a1.83 1.83 0 0 0 1.556-.881 1.785 1.785 0 0 0 0-1.784l-1.226-2.066"/><path d="m14 16-3 3 3 3"/><path d="M8.293 13.596 7.196 9.5 3.1 10.598"/><path d="m9.344 5.811 1.093-1.892A1.83 1.83 0 0 1 11.985 3a1.784 1.784 0 0 1 1.546.888l3.943 6.843"/><path d="m13.378 9.633 4.096 1.098 1.097-4.096"/>',
     ...
     *
     * @param string                 $name  Icon name (zie $LUCIDE_PATHS keys)
     * @param int|array<string,mixed> $opts  Size (int) of options-array
     * @return string                Inline SVG markup
     */
    function pp_icon(string $name, $opts = []): string
    {
        if (is_int($opts)) {
            $opts = ['size' => $opts];
        }
        ...
        $stroke = (string) ($opts['stroke'] ?? '1.75');
        $aria_label = $opts['label'] ?? null;
        $hidden = $aria_label === null;
        $path = $GLOBALS['LUCIDE_PATHS'][$name] ?? null;
        if ($path === null) {
            // Fallback: render een placeholder cirkel zodat layout niet breekt
            $path = '<circle cx="12" cy="12" r="10"/>';
        }

        $attrs = sprintf(
            'width="%d" height="%d" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="%s" stroke-linecap="round" stroke-linejoin="round"',
            $size,
            $size,
        ...
        if ($hidden) {
            ...
            $attrs .= ' role="img" aria-label="' . htmlspecialchars((string) $aria_label, ENT_QUOTES, 'UTF-8') . '"';
        }
    ...
    /**
     * Echo helper rondom pp_icon.
     */
    function pp_icon_e(string $name, $opts = []): void
    {
        echo pp_icon($name, $opts);
    }
}
