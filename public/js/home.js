document.addEventListener("DOMContentLoaded", function () {
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", function (e) {
      ...
      submitBtn.innerHTML =
        '<span class="flex items-center"><span class="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></span>Bezig...</span>';
      submitBtn.disabled = true;

      // Make API call to newsletter subscribe endpoint
      fetch(
        `${window.URLROOT || window.location.origin}/newsletter/subscribe`,
        {
          method: "POST",
          headers: {
        ...
        .catch((error) => {
          ...
          showNotification(
            "Er is een probleem opgetreden. Controleer je internetverbinding en probeer het opnieuw.",
            "error"
          );
        })
        .finally(() => {
  ...
  function showNotification(message, type = "info") {
    ...
    notification.className = `notification fixed top-4 right-4 z-50 px-6 py-4 rounded-lg shadow-lg transform translate-x-full transition-transform duration-300 ${getNotificationClass(
      type
    )}`;
