...
$nederlandse_verkiezingen = $db->resultSet();
    ...
    $fallback_latest_verkiezing = $fallback_data['fallback_latest_verkiezing_2025'] ?? null;
    if ($fallback_latest_verkiezing) {
        $has_latest = false;
        foreach ($nederlandse_verkiezingen as $record) {
            if (!empty($record->jaar) && (int) $record->jaar === (int) $fallback_latest_verkiezing['jaar']) {
                $has_latest = true;
                break;
            }
        }
        ...
        if (!$has_latest) {
            array_unshift($nederlandse_verkiezingen, (object) $fallback_latest_verkiezing);
        }
    }
