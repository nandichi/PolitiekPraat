...
function stemwijzer() {
    return {
        ...
        updatePartyGroups() {
            if (!this.questions || this.questions.length === 0 || this.currentStep >= this.questions.length) {
                return;
            }
            
            // De huidige vraag
            const currentQuestion = this.questions[this.currentStep];
            if (!currentQuestion || !currentQuestion.positions) {
                return;
            }
            ...
            // Partijen sorteren op hun standpunt
            for (const party in currentQuestion.positions) {
                const stance = currentQuestion.positions[party];
                if (stance === 'eens') {
                    this.eensParties.push(party);
                } else if (stance === 'neutraal') {
                    this.neutraalParties.push(party);
                } else if (stance === 'oneens') {
        ...
        skipQuestion() {
            delete this.answers[this.currentStep];
            this.nextQuestion();
        },
        ...
        countAnswerType(type) {
            ...
            for (const questionIndex in this.answers) {
                if (this.answers[questionIndex] === type) {
                    count++;
                }
            }
            return count;
        ...
        calculateResults() {
            ...
            // Voor elke partij berekenen we de overeenkomst
            for (const question of this.questions) {
                for (const party in question.positions) {
                    if (!this.results[party]) {
                        this.results[party] = {
                            name: party,
            ...
            // Voor elke beantwoorde vraag, check de overeenkomst met elke partij
            for (const questionIndex in this.answers) {
                const userAnswer = this.answers[questionIndex];
                const question = this.questions[questionIndex];
                
                for (const party in question.positions) {
                    const partyStance = question.positions[party];
                    
                    // Verhoog het aantal beantwoorde vragen voor deze partij
                    this.results[party].totalAnswered++;
                    
                    // Check voor een exacte match
                    if (userAnswer === partyStance) {
                        this.results[party].matchingAnswers++;
                    }
                    // Check voor een gedeeltelijke match (eens vs neutraal of oneens vs neutraal)
                    else if ((userAnswer === 'eens' && partyStance === 'neutraal') || 
                             (userAnswer === 'neutraal' && partyStance === 'eens') ||
                             (userAnswer === 'oneens' && partyStance === 'neutraal') ||
                             (userAnswer === 'neutraal' && partyStance === 'oneens')) {
                        this.results[party].matchingAnswers += 0.5;
                    }
                }
            }
            
            // Bereken het overeenkomstpercentage
            for (const party in this.results) {
                if (this.results[party].totalAnswered > 0) {
                    const matchPercentage = (this.results[party].matchingAnswers / this.results[party].totalAnswered) * 100;
                    this.results[party].agreement = Math.round(matchPercentage);
                }
            }
            
            // Zet de resultaten om naar een array en sorteer op overeenkomst (hoog naar laag)
            this.finalResults = Object.values(this.results).sort((a, b) => b.agreement - a.agreement);
        },
