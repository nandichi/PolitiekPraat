...
// Database verbinding initialiseren
$db = new Database();

// Haal alle vragen op
$db->query("SELECT * FROM questions ORDER BY question_id");
$questionsData = $db->resultSet();

// Haal alle partijen op
$db->query("SELECT * FROM parties ORDER BY party_id");
$partiesData = $db->resultSet();

// Haal alle posities op
$db->query("SELECT * FROM positions");
$positionsData = $db->resultSet();

// Bereid de datastructuur voor die we naar JavaScript zullen sturen
$questions = [];
$partyLogos = [];

// Partijlogo's opslaan
foreach ($partiesData as $party) {
    $partyLogos[$party->party_name] = $party->party_logo;
}

// Posities organiseren per vraag en partij
$positionsMap = [];
foreach ($positionsData as $position) {
    $positionsMap[$position->question_id][$position->party_id] = [
        'stance' => $position->stance,
        'explanation' => $position->explanation
    ];
}

// Vragen opbouwen met bijbehorende posities
foreach ($questionsData as $question) {
    ...
    // Voeg posities toe voor elke partij
    foreach ($partiesData as $party) {
        if (isset($positionsMap[$question->question_id][$party->party_id])) {
            $position = $positionsMap[$question->question_id][$party->party_id];
            $questionItem['positions'][$party->party_name] = $position['stance'];
            $questionItem['explanations'][$party->party_name] = $position['explanation'];
        }
    }
...
function stemwijzer() {
    return {
        screen: 'start',
        currentStep: 0,
        totalSteps: <?php echo count($questions); ?>,
        ...
        partyLogos: <?php echo json_encode($partyLogos); ?>,
